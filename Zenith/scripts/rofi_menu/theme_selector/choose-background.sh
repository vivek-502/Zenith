...
# ---- Paths ----
WALL_DIR="$HOME/.config/Zenith/backgrounds"
ROFI_THEME="$HOME/.config/rofi/theme-switcher.rasi"
CACHE_DIR="$HOME/.cache/rofi_wallpaper_thumbs"
...
while IFS=$'\t' read -r filename fullpath; do
    ...
    if [[ ! -f "$thumb_path" ]]; then
        magick "$fullpath" -resize 1920x1080^ -gravity center -extent 1920x1080 "$thumb_path"
    fi

    # Formatting for Rofi: Icon + Path Info
    entries+="$filename\0icon\x1f$thumb_path\x1finfo\x1f$fullpath\n"

done < <(find "$WALL_DIR" -type f -iregex '.*\.\(jpg\|jpeg\|png\|webp\)' -printf "%f\t%p\n" | sort -f)
...
if [[ -f "$FULL_PATH" ]]; then
    ...
    (
        cp "$FULL_PATH" "$HOME/.config/Zenith/current/background.jpeg"
        magick "$FULL_PATH" -resize 530x990^ -gravity center -extent 530x990 "$HOME/.config/Zenith/current/rofi-vertical.jpeg"
        magick "$FULL_PATH" -resize 930x530^ -gravity center -extent 930x530 "$HOME/.config/Zenith/current/rofi-horizontal.jpeg"

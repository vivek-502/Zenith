...
EXTRA_STYLES_DIR="$HOME/.config/Zenith/extra-styles/rofi/powermenu"
...
CHOICE=$(echo -e "$OPTIONS" | rofi -dmenu -i \
    ...
    -theme-str 'entry { placeholder: "Select Powermenu style..."; }')
...
if [[ -n "$CHOICE" ]]; then
    ...
    if [[ -d "$STYLES_DIR/$STYLE_NAME" ]]; then
        ...
        cp -rf "$SELECTED_PATH/rofi/powermenu.rasi" "$TARGET_DIR/"
        cp -rf "$SELECTED_PATH/rofi/animations/Power_menu_anim.conf" "$TARGET_DIR/animations/"
        ...
        cp -rf "$SELECTED_PATH/powermenu.rasi" "$TARGET_DIR/"
        cp -rf "$SELECTED_PATH/Power_menu_anim.conf" "$TARGET_DIR/animations/"

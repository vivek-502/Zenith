...
EXTRA_STYLES_DIR="$HOME/.config/Zenith/extra-styles/waybar"
...
CHOICE=$(echo -e "$OPTIONS" | rofi -dmenu -i \
    ...
    -theme-str 'entry { placeholder: "Select Waybar style..."; }')
...
if [[ -n "$CHOICE" ]]; then
    ...
    if [[ -d "$STYLES_DIR/$STYLE_NAME" ]]; then
        ...
        cp -rf "$SELECTED_PATH/waybar/config.jsonc" "$TARGET_DIR/"
        cp -rf "$SELECTED_PATH/waybar/style.css" "$TARGET_DIR/"
                
    elif [[ -d "$EXTRA_STYLES_DIR/$STYLE_NAME" ]]; then
        SELECTED_PATH="$EXTRA_STYLES_DIR/$STYLE_NAME"
        
        cp -rf "$SELECTED_PATH/config.jsonc" "$TARGET_DIR/"
        cp -rf "$SELECTED_PATH/style.css" "$TARGET_DIR/"
    ...
    pkill waybar && waybar
    notify-send -a "Appearance" "Waybar Style Applied"
fi

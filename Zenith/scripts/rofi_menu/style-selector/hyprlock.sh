...
EXTRA_STYLES_DIR="$HOME/.config/Zenith/extra-styles/hyprlock"
...
CHOICE=$(echo -e "$OPTIONS" | rofi -dmenu -i \
    ...
    -theme-str 'entry { placeholder: "Select hyprlock style..."; }')
...
if [[ -n "$CHOICE" ]]; then
    ...
    if [[ -d "$STYLES_DIR/$STYLE_NAME" ]]; then
        ...
        cp -rf "$SELECTED_PATH/hyprland/hyprlock.conf" "$TARGET_DIR/"
                
    elif [[ -d "$EXTRA_STYLES_DIR/$STYLE_NAME" ]]; then
        SELECTED_PATH="$EXTRA_STYLES_DIR/$STYLE_NAME"
        
        cp -rf "$SELECTED_PATH/hyprlock.conf" "$TARGET_DIR/"
    ...
    notify-send -a "Appearance" "Hyprlock Style Applied"
fi

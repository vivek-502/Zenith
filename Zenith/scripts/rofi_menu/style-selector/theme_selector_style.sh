...
EXTRA_STYLES_DIR="$HOME/.config/Zenith/extra-styles/rofi/themeselector"
...
CHOICE=$(echo -e "$OPTIONS" | rofi -dmenu -i \
    ...
    -theme-str 'entry { placeholder: "Select Theme selector style..."; }')
...
if [[ -n "$CHOICE" ]]; then
    ...
    if [[ -d "$STYLES_DIR/$STYLE_NAME" ]]; then
        ...
        cp -rf "$SELECTED_PATH/rofi/theme-switcher.rasi" "$TARGET_DIR/"
        cp -rf "$SELECTED_PATH/rofi/animations/Theme_selector_anim.conf" "$TARGET_DIR/animations/"
        ...
        cp -rf "$SELECTED_PATH/theme-switcher.rasi" "$TARGET_DIR/"
        cp -rf "$SELECTED_PATH/Theme_selector_anim.conf" "$TARGET_DIR/animations/"

...
EXTRA_STYLES_DIR="$HOME/.config/Zenith/extra-styles/rofi/applauncher"
...
CHOICE=$(echo -e "$OPTIONS" | rofi -dmenu -i \
    ...
    -theme-str 'entry { placeholder: "Select App launcher style..."; }')
...
if [[ -n "$CHOICE" ]]; then
    ...
    if [[ -d "$STYLES_DIR/$STYLE_NAME" ]]; then
        ...
        cp -rf "$SELECTED_PATH/rofi/config.rasi" "$TARGET_DIR/"
        cp -rf "$SELECTED_PATH/rofi/animations/App_launcher_anim.conf" "$TARGET_DIR/animations/"
        ...
        cp -rf "$SELECTED_PATH/config.rasi" "$TARGET_DIR/"
        cp -rf "$SELECTED_PATH/App_launcher_anim.conf" "$TARGET_DIR/animations/"
